...
AddEventHandler('cyber:camping',function()
    ...
    if IsPedInAnyVehicle(b)then 
        exports['ttyy_nuipack']:DoHudText('inform','This operation cannot be performed in the vehicle.')
    else 
        if d<9999999999999999999999 then 
            crouch()
            ...
            exports['ttyy_nuipack']:DoHudText('inform','You can camp only in designated areas.')
        end 
    end 
end)
    
    RegisterNetEvent("cyber-campingcreateobject")
    AddEventHandler("cyber-campingcreateobject",function(f,g,h)
        local i=PlayerPedId()
    ...
    while true do
        ...
        if DoesEntityExist(entity) and #(pcoords-entityCoords) < 3.5 then
            if isindim == false then
                startAnim()
                exports['ttyy_nuipack']:SendAlert('success', 'You are warming up')
                Citizen.Wait(1000)
                isindim = true
                isinmadim = false
            end
            ...
            if isindim == true and isinmadim == false then
                exports['ttyy_nuipack']:SendAlert('error', 'You went away from the fire!')
                ClearPedTasks(ped)
                isindim = false
                isinmadim = true
            end
