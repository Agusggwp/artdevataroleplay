...
Fuel.JerryCanCost = 5000
Fuel.RefillCost = 2500 -- If it is missing half of it capacity, this amount will be divided in half, and so on.

-- Fuel decor - No need to change this, just leave it.
Fuel.FuelDecor = "_FUEL_LEVEL"

-- What keys are disabled while you're fueling.
Fuel.DisableKeys = {0, 22, 23, 24, 29, 30, 31, 37, 44, 56, 82, 140, 166, 167, 168, 170, 288, 289, 311, 323}

-- Want to use the HUD? Turn this to true.
Fuel.EnableHUD = false

-- Fuelure blips here. Turn both to false to disable blips all together.
Fuel.ShowNearestGasStationOnly = true
Fuel.ShowAllGasStations = false

-- Modify the fuel-cost here, using a multiplier value. Setting the value to 2.0 would cause a doubled increase.
Fuel.CostMultiplier = 1.0

-- Fuelure the strings as you wish here.
Fuel.Strings = {
	...
	PurchaseJerryCan = "Press ~g~E ~w~to purchase a jerry can for ~g~$" .. Fuel.JerryCanCost,
...
if not Fuel.UseESX then
	Fuel.Strings.PurchaseJerryCan = "Press ~g~E ~w~to grab a jerry can"
	Fuel.Strings.RefillJerryCan = "Press ~g~E ~w~ to refill the jerry can"
end
...
Fuel.RemoveHUDForBlacklistedVehicle = true
