...
AddEventHandler(
    ...
    function(id, job)
        ...
        if xPlayer then
            TriggerClientEvent("core_jobutilities:sendMessage", xPlayer.source, Config_Core.Text["hired"])
            TriggerClientEvent("core_jobutilities:sendMessage", src, Config_Core.Text["action_success"])
            xPlayer.setJob(job, 0)
            ...
            TriggerClientEvent("core_jobutilities:sendMessage", src, Config_Core.Text["action_unsuccessful"])
        end
    end
)
    ...
    function(identifier, job)
        ...
        if xPlayer then
            if xPlayer.getJob().name == job then
                ...
                TriggerClientEvent("core_jobutilities:sendMessage", xPlayer.source, Config_Core.Text["fired"])
                TriggerClientEvent("core_jobutilities:sendMessage", src, Config_Core.Text["action_success"])
            else
                TriggerClientEvent("core_jobutilities:sendMessage", xPlayer.source, Config_Core.Text["fired_other"])
            end
...
AddEventHandler(
    ...
    account.addMoney( tonumber(amount))
            ...
            TriggerClientEvent("core_jobutilities:sendMessage", src, Config_Core.Text["insufficent_balance"])
        end
    end
)
...
AddEventHandler(
    ...
    function(identifier, amount, job)
        ...
        if xPlayer then
                ...
                TriggerClientEvent("core_jobutilities:sendMessage", src, Config_Core.Text["bonus_given"])
                TriggerClientEvent("core_jobutilities:sendMessage", xPlayer.source, Config_Core.Text["bonus_recieved"] .. amount)
        else
    ...
    function(identifier, job, rank)
        ...
        if xPlayer then
            if xPlayer.getJob().name == job then
                ...
                TriggerClientEvent("core_jobutilities:sendMessage", src, Config_Core.Text["action_success"])
                TriggerClientEvent("core_jobutilities:sendMessage", xPlayer.source, Config_Core.Text["promoted"])
            else
                TriggerClientEvent("core_jobutilities:sendMessage", xPlayer.source, Config_Core.Text["promoted_other"])
            end
        end
            MySQL.Async.execute(
                "UPDATE users SET `job`=@job, `job_grade`=@rank WHERE `identifier` = @identifier",
