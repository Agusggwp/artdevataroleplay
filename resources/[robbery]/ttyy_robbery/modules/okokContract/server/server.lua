			...
			if bankMoney >= vehicle_price then
				MySQL.Async.execute('UPDATE owned_vehicles SET owner = @target WHERE owner = @owner AND plate = @plate', {
					...
					if result2 ~= 0 then	
						...
						TriggerClientEvent('ttyy_nuipack:Alert', _source, "VEHICLE", "You successfully sold the vehicle <b>"..model.."</b> with the plate number <b>"..plate.."</b>", 10000, 'success')
						TriggerClientEvent('ttyy_nuipack:Alert', target, "VEHICLE", "You successfully bought the vehicle <b>"..model.."</b> with the plate number <b>"..plate.."</b>", 10000, 'success')
				...
				TriggerClientEvent('ttyy_nuipack:Alert', _source, "VEHICLE", target_name.." doesn't have enough money to buy your vehicle", 10000, 'error')
				TriggerClientEvent('ttyy_nuipack:Alert', target, "VEHICLE", "You don't have enough money to buy "..source_name.."'s vehicle", 10000, 'error')
			end
		else
			TriggerClientEvent('ttyy_nuipack:Alert', _source, "VEHICLE", "The vehicle with the plate number <b>"..plate.."</b> isn't yours", 10000, 'error')
			TriggerClientEvent('ttyy_nuipack:Alert', target, "VEHICLE", source_name.." tried to sell you a vehicle he doesn't owns", 10000, 'error')
		end
	else
		if result[1] ~= nil then
			MySQL.Async.execute('UPDATE owned_vehicles SET owner = @target WHERE owner = @owner AND plate = @plate', {
	...
	local information = {
		{
			...
			["author"] = {
				...
				["name"] = Kontraksu.ServerName..' - Logs',
			},
			["title"] = 'VEHICLE SALE',
