...
Citizen.CreateThread(function()
	...
	local storageConfig = Config_Storage.Storages
...
TriggerEvent('zf_context:openMenu', {
        ...
        },
        {
            ...
            txt = 'Purchase this unit for $' ..Config_Storage.UnitPrice,
            params = {
...
AddEventHandler('koe_storageunits:policeBreach', function(data)
    ...
    for k, v in pairs(Config_Storage.Policeraid.Jobs) do
    ...
    for k, v in pairs(Config_Storage.Policeraid.Jobs) do
        if v.job == ESX.PlayerData.job.name and ESX.PlayerData.job.grade < v.grade then
            exports['ttyy_nuipack']:Alert("Storage", "Not a high enough rank to do that.", 8000, 'error')
        end
    end 
    if ESX.PlayerData.job.name ~= 'police' then
        exports['ttyy_nuipack']:Alert("Storage", "You cant do that, youre not a cop.", 8000, 'error')
    end  
end)
