...
AddEventHandler('koe_storageunits:buyUnit', function(storageID)
    ...
    function(result2) 
      -- print(json.encode(result2))
            if xPlayer.getMoney() >= Config_Storage.UnitPrice then
              MySQL.Async.fetchAll("UPDATE storageunits SET identifier = @identifier WHERE id =@id",{['@identifier']  = identifier, ['@id'] = storageID}, function(result)
                xPlayer.removeMoney(Config_Storage.UnitPrice)
            end)
                TriggerClientEvent('ttyy_nuipack:Alert', src, "Storage", "You now own this storage unit", 10000, 'success')
            else    
              TriggerClientEvent('ttyy_nuipack:Alert', src, "Storage", "Not enough money", 10000, 'error')
            end
...
AddEventHandler('koe_storageunits:sellUnit', function(storageID)
    ...
    function(result2) 
      -- print(json.encode(result2))
              ...
              MySQL.Async.fetchAll("UPDATE storageunits SET identifier = @identifier WHERE id =@id",{['@identifier']  = identifier, ['@id'] = storageID}, function(result)
                xPlayer.addMoney(Config_Storage.SellPrice)
              end)
                TriggerClientEvent('ttyy_nuipack:Alert', src, "Storage", "You sold the unit", 10000, 'info')
    end)
end)
