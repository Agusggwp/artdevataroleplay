...
RegisterNetEvent("ttyy-fleeca:resetDoorState")
AddEventHandler("ttyy-fleeca:resetDoorState", function(name)
    Freeze[name] = 0
end)

RegisterNetEvent("ttyy-fleeca:lootup_c")
AddEventHandler("ttyy-fleeca:lootup_c", function(var, var2)
    LootCheck[var][var2] = true
end)

RegisterNetEvent("ttyy-fleeca:outcome")
AddEventHandler("ttyy-fleeca:outcome", function(oc, arg)
    for i = 1, #Check, 1 do
        Check[i] = false
    end
    ...
    if oc then
        Check[arg] = true
        TriggerEvent("ttyy-fleeca:startheist", Config_Fleeca.Banks[arg], arg)
    elseif not oc then
		TriggerEvent('ox_inventory:notify', {type = 'error', arg})
    end
end)

RegisterNetEvent("ttyy-fleeca:startLoot_c")
AddEventHandler("ttyy-fleeca:startLoot_c", function(data, name)
            ...
            while true do
                ...
                if dst < 40 then
                    ...
                    if LootCheck[name].Stop or (LootCheck[name].Loot1 and LootCheck[name].Loot2 and LootCheck[name].Loot3) then
                        ...
                        if initiator then
                            TriggerEvent("ttyy-fleeca:reset", name, data)
                            return
                        end
                        return
                    end
...
RegisterNetEvent("ttyy-fleeca:stopHeist_c")
AddEventHandler("ttyy-fleeca:stopHeist_c", function(name)
    LootCheck[name].Stop = true
end)

-- MAIN DOOR UPDATE --

AddEventHandler("ttyy-fleeca:freezeDoors", function()
    Citizen.CreateThread(function()
        while true do
...
RegisterNetEvent("ttyy-fleeca:toggleVault")
AddEventHandler("ttyy-fleeca:toggleVault", function(key, state)
    dooruse = true
    ...
    if Config_Fleeca.Banks[key].hash == nil then
        if not state then
            ...
            TriggerServerEvent("ttyy-fleeca:updateVaultState", key, Doors[key][2].state)
        elseif state then
            local obj = GetClosestObjectOfType(Config_Fleeca.Banks[key].doors.startloc.x, Config_Fleeca.Banks[key].doors.startloc.y, Config_Fleeca.Banks[key].doors.startloc.z, 2.0, GetHashKey(Config_Fleeca.vaultdoor), false, false, false)
            local count = 0
...
AddEventHandler("ttyy-fleeca:reset", function(name, data)
    ...
    TriggerServerEvent("ttyy-fleeca:toggleVault", name, true)
    TriggerEvent("ttyy-fleeca:cleanUp", data, name)
end)

AddEventHandler("ttyy-fleeca:startheist", function(data, name)
    TriggerServerEvent("ttyy-fleeca:toggleDoor", name, true) -- ensure to lock the second door for the second, third, fourth... heist
    disableinput = true
    currentname = name
    currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    initiator = true
    ...
    TriggerServerEvent("ttyy-fleeca:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    SpawnTrolleys(data, name)
end)

AddEventHandler("ttyy-fleeca:cleanUp", function(data, name)
    Citizen.Wait(10000)
    ...
    TriggerServerEvent("ttyy-fleeca:setCooldown", name)
    initiator = false
end)
...
function SpawnTrolleys(data, name)
    ...
    TriggerServerEvent("ttyy-fleeca:startLoot", data, name, missionplayers)
    done = false
end

RegisterNetEvent('ttyy-fleeca:grab')
AddEventHandler('ttyy-fleeca:grab', function(name)
    disableinput = true
    local ped = PlayerPedId()
		    ...
		    while GetGameTimer() - startedGrabbing < 37000 do
			    ...
			    if HasAnimEventFired(ped, `RELEASE_CASH_DESTROY`) then
				    if IsEntityVisible(grabobj) then
                        SetEntityVisible(grabobj, false, false)
                        TriggerServerEvent("ttyy-fleeca:rewardCash")
				    end
			    end
		    end
        ...
        if startdstcheck then
            if initiator then
                ...
                if (GetDistanceBetweenCoords(playercoord, currentcoords, true)) > 20 then
                    ...
                    startdstcheck = false
                    TriggerServerEvent("ttyy-fleeca:stopHeist", currentname)
                end
            end
            Citizen.Wait(1)
        else
...
Citizen.CreateThread(function()
    ...
    ESX.TriggerServerCallback("ttyy-fleeca:getBanks", function(bank, door)
        Config_Fleeca.Banks = bank
        Doors = door
    end)
    TriggerEvent("ttyy-fleeca:freezeDoors")
    while true do
...
RegisterNetEvent('ttyy-fleeca:hack')
AddEventHandler('ttyy-fleeca:hack', function()
    local coords = GetEntityCoords(PlayerPedId())
    ...
    for k, v in pairs(Config_Fleeca.Banks) do
        if not v.onaction then
            ...
            if dst <= 1 and not Check[k] then
                Wait(750)
                exports['ttyy_nuipack']:OpenHackingGame(function(outcome)
                    if outcome == true then 
                        Wait(750)
                        TriggerServerEvent("ttyy-fleeca:startcheck", k)
...
RegisterNetEvent("ttyy-fleeca:policenotify")
AddEventHandler("ttyy-fleeca:policenotify", function(name)
