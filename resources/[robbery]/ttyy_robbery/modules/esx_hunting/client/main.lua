...
RegisterNetEvent('ttyy-hunt:starthunting')
AddEventHandler('ttyy-hunt:starthunting', function()
	StartHuntingSession()
end)

RegisterNetEvent('ttyy-hunt:stophunting')
AddEventHandler('ttyy-hunt:stophunting', function()
	StopHuntingSession()
end)

RegisterNetEvent('ttyy-hunt:slaugther')
AddEventHandler('ttyy-hunt:slaugther', function(AnimalId)
...
RegisterNetEvent('ttyy-hunt:huntingsell')
AddEventHandler('ttyy-hunt:huntingsell', function()
	SellItems()
end)
...
Citizen.CreateThread(function()
	Citizen.CreateThread(function()
		...
		exports['qtarget']:AddTargetModel(pedshop, {
			options = {
				{
					event = "ttyy-hunt:starthunting",
					icon = "fas fa-bullseye",
					label = "Start Hunting",
				},
				{
					event = "ttyy-hunt:stophunting",
					icon = "fas fa-bullseye",
					label = "Stop Hunting",
				},
				{
		...
		exports['qtarget']:AddTargetModel(pedsanimal, {
			options = {
				{
					event = "ttyy-hunt:slaugther",
					icon = "fas fa-bullseye",
		...
		exports['qtarget']:AddBoxZone("sellhunt", vector3(929.74, -1807.56, 31.31), 1, 2.2, {
			name="sellhunt",
			heading=265,
			--debugPoly=true,
			minZ=29.11,
...
function StartHuntingSession()
	...
	if OnGoingHuntSession == true then
		exports['ttyy_nuipack']:DoHudText('error', 'you already in this job!' )
	else
		OnGoingHuntSession = true

		--Car
		HuntCar = CreateVehicle(GetHashKey('akuma'), Positions['SpawnATV'].x, Positions['SpawnATV'].y, Positions['SpawnATV'].z, 169.79, true, false)
...
function SlaughterAnimal(AnimalId)
	...
	exports['rprogress']:Start("Menguliti Sapi", 5000)
	Citizen.Wait(5000)
