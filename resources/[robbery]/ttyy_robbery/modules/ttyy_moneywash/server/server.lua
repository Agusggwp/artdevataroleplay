...
AddEventHandler('norp-moneywash:canWashMoney',function(amountToWash)
    ...
    if bm >= MoneyWashCok.MinAmountToWash and xPlayer.getAccount('black_money').money >= bm then
        xPlayer.removeAccountMoney('black_money', amountToWash)
        TriggerClientEvent("norp-moneywash:MoneyWashFunc", source, amountToWash)
    else
		TriggerClientEvent('ttyy_notify:Alert', source, "Money Laundering", "You dont have enough Black Money to wash it!", 5000, 'error')
    end
end)
...
AddEventHandler('norp-moneywash:washMoney',function(amountToWash)
	local xPlayer = ESX.GetPlayerFromId(source)
    if MoneyWashCok.EnableTax then
        local tax = MoneyWashCok.TaxRate
        local delete = amountToWash / 100 * tax
        local clean = amountToWash - delete
        xPlayer.addMoney(clean)
		TriggerClientEvent('ttyy_notify:Alert', source, "Money Laundering", "Your money was laundered, you got: $"..clean.."!", 5000, 'success')
    else
        xPlayer.addMoney(amountToWash)
		TriggerClientEvent('ttyy_notify:Alert', source, "Money Laundering", "Your money was laundered, you got: $"..amountToWash.."!", 5000, 'info')
    end
end)
