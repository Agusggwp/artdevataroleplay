...
next_ped = function(drugToSell)
	...
	if cooldown then
		ESX.ShowAdvancedNotification(Config_Selldrugs.notify.title, '', Config_Selldrugs.notify.cooldown, 'DIA_CLIFFORD', 1)
		return
	end

	cooldown = true

	if Config_Selldrugs.cityPoint ~= false and GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config_Selldrugs.cityPoint, true) > 1500.0 then
		ESX.ShowAdvancedNotification(Config_Selldrugs.notify.title, '', Config_Selldrugs.notify.toofar, 'DIA_CLIFFORD', 1)
		return
	end
	...
	if cops < Config_Selldrugs.requiredCops then
		ESX.ShowAdvancedNotification(Config_Selldrugs.notify.title, '', Config_Selldrugs.notify.cops, 'DIA_CLIFFORD', 1)
		return
	end
	...
	ESX.ShowAdvancedNotification(Config_Selldrugs.notify.title, '', Config_Selldrugs.notify.searching .. drugToSell.label, 'DIA_CLIFFORD', 1)
	Wait(math.random(5000, 10000))
	--ESX.PlayAnim('amb@world_human_drug_dealer_hard@male@base', 'base', 8.0, -1, 1)
        ClearPedTasks(PlayerPedId()) 
	npc.hash = GetHashKey(Config_Selldrugs.pedlist[math.random(1, #Config_Selldrugs.pedlist)])
	...
	if retval == false then
		cooldown = false
		ESX.ShowAdvancedNotification(Config_Selldrugs.notify.title, '', Config_Selldrugs.notify.abort, 'DIA_CLIFFORD', 1)
		ClearPedTasks(PlayerPedId())
		return
	end
	...
	if IsEntityDead(npc.ped) or GetEntityCoords(npc.ped) == vector3(0.0, 0.0, 0.0) then
		ESX.ShowAdvancedNotification(Config_Selldrugs.notify.title, '', Config_Selldrugs.notify.notfound, 'DIA_CLIFFORD', 1)
        return
	end
	
	ESX.ShowAdvancedNotification(Config_Selldrugs.notify.title, Config_Selldrugs.notify.approach, Config_Selldrugs.notify.found .. npc.zone, 'DIA_CLIFFORD', 1)
	...
	CreateThread(function()
		...
		while npc.ped ~= nil and npc.ped ~= 0 and not IsEntityDead(npc.ped) do
			...
			ESX.Game.Utils.DrawText3D(npc.coords, (Config_Selldrugs.notify.client):format(drugToSell.count, drugToSell.label), 0.5)
			distance = Vdist2(GetEntityCoords(PlayerPedId()), npc.coords)
			
			if distance < 2.0 then
				ESX.ShowHelpNotification(Config_Selldrugs.notify.press)
				if IsControlJustPressed(0, 38) and canSell then
					canSell = false
					reject = math.random(1, 10)

					if reject <= 3 then
						ESX.ShowAdvancedNotification(Config_Selldrugs.notify.title, '', Config_Selldrugs.notify.reject, 'DIA_CLIFFORD', 1)
						...
						SetPedAsNoLongerNeeded(npc.ped)
						if Config_Selldrugs.npcFightOnReject then
					...
					if IsPedInAnyVehicle(PlayerPedId(), false) then
						ESX.ShowAdvancedNotification(Config_Selldrugs.notify.title, Config_Selldrugs.notify.vehicle, '', 'DIA_CLIFFORD', 1)
						return
					end
					...
					TriggerServerEvent('met', drugToSell)
					ESX.ShowAdvancedNotification(Config_Selldrugs.notify.title, '', (Config_Selldrugs.notify.sold):format(drugToSell.count, drugToSell.label, drugToSell.price), 'DIA_CLIFFORD', 1)
					npc = {}
				end
			end
		end
...
AddEventHandler('stasiek_selldrugsv2:notifyPolice', function(coords)	
	if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
		...
		ESX.ShowAdvancedNotification(Config_Selldrugs.notify.police_notify_title, Config_Selldrugs.notify.police_notify_subtitle, street2, "CHAR_CALL911", 1)
...
exports['qtarget']:AddBoxZone("startjual", vector3(-1266.24, -1429.84, 4.35), 1, 2.8, {
	name="startjual",
	heading=305,
	--debugPoly=true,
	minZ=2.55,
...
RegisterNetEvent('drugs')
AddEventHandler('drugs', function ()
