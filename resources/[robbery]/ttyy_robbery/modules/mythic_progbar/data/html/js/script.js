...
$('document').ready(function() {
    ...
    MythicProgBar.Progress = function(data) {
        ...
        $(".progress-container").fadeIn('fast', function() {
            $("#progress-bar").stop().css({"width": 0, "background-color": "rgba(0, 246, 215, 0.75)"}).animate({
              width: '100%'
            }, {
              ...
              complete: function() {
                $(".progress-container").fadeOut('fast', function() {
                    ...
                    $.post('http://ttyy_robbery/actionFinish', JSON.stringify({
                        })
                    );
                })
              }
    ...
    MythicProgBar.ProgressCancel = function() {
        ...
        cancelledTimer = setTimeout(function () {
            $(".progress-container").fadeOut('fast', function() {
                $("#progress-bar").css("width", 0);
                $.post('http://ttyy_robbery/actionCancel', JSON.stringify({
                    })
                );
            });
        }, 1000);
