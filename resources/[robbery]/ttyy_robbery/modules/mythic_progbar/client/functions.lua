AddEventHandler('esx:nui_ready', function()
	print('TTYY mythic_progbar READY')
    BuatFrame('mythic_progbar', 'nui://' .. GetCurrentResourceName() .. '/modules/mythic_progbar/data/html/ui.html')
end)
...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or mythic_action.useWhileDead then
        if not isDoingAction then
            ...
            KirimFrameMessage('mythic_progbar', {
            ...
            Citizen.CreateThread(function ()
                ...
                while isDoingAction do
                    ...
                    if IsControlJustPressed(0, 178) and mythic_action.canCancel then
                        TriggerEvent("ttyy_robbery:client:cancel")
                    end
            ...
            exports['ttyy_nuipack']:SendAlert('error', 'Already Doing An Action', 1000)
        end
    else
        exports['ttyy_nuipack']:SendAlert('error', 'Cannot Perform An Action While Dead', 1000)
    end
end
