...
ESX.RegisterServerCallback('av_bobcat:c4', function(source,cb)
	...
	if xPlayer.getInventoryItem(Config_Bobcat.C4Name).count >= Config_Bobcat.C4amount then
		xPlayer.removeInventoryItem(Config_Bobcat.C4Name,Config_Bobcat.C4amount)
		cb(true)
	else
		TriggerClientEvent('av_bobcat:notify',source,Config_Bobcat.Lang['missing_c4'])
		cb(false)
	end	
end)
...
ESX.RegisterServerCallback('av_bobcat:cooldown', function(source,cb)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == Config_Bobcat.PoliceJobName then
			cops = cops + 1
		end		
	end
		
	if cops >= Config_Bobcat.MinPolice and not esperar then
		if Config_Bobcat.NeedsBlowtorch then
			if xPlayer.getInventoryItem(Config_Bobcat.BlowtorchName).count >= 1 then	
				if Config_Bobcat.RemoveBlowtorch then
					xPlayer.removeInventoryItem(Config_Bobcat.BlowtorchName,1)
				end
				cb(false)			
			else
				cb(true)
		...
		TriggerClientEvent('av_bobcat:notify',source,Config_Bobcat.Lang['not_enough'])
		cb(true)
	end
end)
...
AddEventHandler('av_bobcat:MakeItRain', function(z)
	...
	local xPlayer = ESX.GetPlayerFromId(source)
	local dinero = Config_Bobcat.Money
	carritos = carritos + carritos
	if carritos <= 3 then
		xPlayer.addAccountMoney(Config_Bobcat.Account,dinero)
		TriggerClientEvent('av_bobcat:notify',source,Config_Bobcat.Lang['stole']..dinero)
	end
end)

Citizen.CreateThread(function()
	local t = Config_Bobcat.CooldownTime * 60
	local w = 10000
	while true do		
		Citizen.Wait(wait)
		if esperar then
