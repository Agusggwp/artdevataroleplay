...
AddEventHandler('hayden_store:robClerk', function(i, id)  
    ...
    if not Config.NPC[i]['Robbed'] then
        if pcountPolice >= Server.RequiredCops then 
            TriggerClientEvent('ttyy_nuipack:client:SendAlert', source, { type = 'success', text = Translation[Config.Language]['playerRobbing'], length = 2500 })
            ...
            TriggerClientEvent('ttyy_nuipack:client:SendAlert', source, { type = 'error', text = Translation[Config.Language]['no_cop'], length = 2500 })
        end 
    else 
        TriggerClientEvent('ttyy_nuipack:client:SendAlert', source, { type = 'error', text = Translation[Config.Language]['recent'], length = 2500 })
    end 
end)
...
AddEventHandler('hayden_store:reward', function(source, i)
    ...
    if (#pCoords - #sCoords) < 10 then
        if hasWeapon() then 
            TriggerClientEvent('ttyy_nuipack:client:SendAlert', source, { type = 'success', text = Translation[Config.Language]['success'], length = 2500 })
            ...
            TriggerEvent('hayden_store:cooldown', i)
            TriggerClientEvent('ttyy_nuipack:client:SendAlert', source, { type = 'error', text = Translation[Config.Language]['nowep'], length = 2500 })
