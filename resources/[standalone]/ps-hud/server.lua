...
-- ESX Callback for ps-hud to check admin status
ESX.RegisterServerCallback('hud:server:getRank', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    local isAdmin = false
    
    if xPlayer then
        isAdmin = IsPlayerAceAllowed(source, 'command') or xPlayer.getGroup() == 'admin'
    end
    
    cb(isAdmin)
end)

-- ESX Callback to get menu configuration
ESX.RegisterServerCallback('hud:server:getMenu', function(source, cb)
    cb(Menu or {})
end)

-- Handle stress gain from client
RegisterNetEvent('hud:server:GainStress', function(amount)
    -- Placeholder for ESX stress system
    -- You can integrate with your ESX status system if available
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer then
        -- Add stress logic here if your server has a stress system
        TriggerClientEvent('chat:addMessage', source, {
            color = {255, 0, 0},
            multiline = true,
            args = {"ps-hud", "Stress increased"}
        })
    end
end)
...
RegisterNetEvent('hud:server:saveUIData', function(data)
    local src = source
	-- Check Permissions (ESX version)
    local xPlayer = ESX.GetPlayerFromId(src)
    if not xPlayer or not (IsPlayerAceAllowed(src, 'command') or xPlayer.getGroup() == 'admin') then
		return
	end
