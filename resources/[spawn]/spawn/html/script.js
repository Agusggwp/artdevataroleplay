...
$(document).on('click', '.location', function(evt){
    ...
    if (type !== "lab") {
        ...
        $.post('http://spawn/setCam', JSON.stringify({
            posname: location,
            type: type,
        }));
...
$(document).on('click', '#submit-spawn', function(evt){
    ...
    if (spawnType !== "appartment") {
        $.post('http://spawn/spawnplayer', JSON.stringify({
            spawnloc: location,
            typeLoc: spawnType
        }));
    } else {
        $.post('http://spawn/chooseAppa', JSON.stringify({
            appType: location,
        }));
    }
});
...
function setupLocations(locations, myHouses) {
    ...
    setTimeout(function(){
        $(parent).append('<div class="location" id="location" data-location="current" data-type="current" data-label="Last Position"><p><span id="current-location">Last Position</span></p></div>');
