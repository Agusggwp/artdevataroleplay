...
function ToggleSpectate(targetPed, targetPlayerId)
    ...
    if SpectateEnabled then
        ...
        clearGamerTagInfo()
        DoScreenFadeIn(500)
        ESX.ShowNotification(Lang:t('spectate.toggled', {toggled_spectate = Lang:t('commands.stopped'), player = GetPlayerName(StoredTargetPlayerId)..' ('..GetPlayerServerId(StoredTargetPlayerId)..')'}))
        StoredTargetPed = nil
    else
        ...
        DoScreenFadeIn(500)
        ESX.ShowNotification(Lang:t('spectate.toggled', {toggled_spectate = Lang:t('commands.started'), player = GetPlayerName(StoredTargetPlayerId)..' ('..GetPlayerServerId(StoredTargetPlayerId)..')'}))
...
function cleanupFailedResolve()
    ...
    ESX.ShowNotification(Lang:t('spectate.stopped_not_found'))
end
...
RegisterNetEvent('Mercy/client/specPlayer', function(targetServerId, coords)
    ...
    if targetPlayerId == PlayerId() then
        return ESX.ShowNotification(Lang:t('spectate.self'))
    end
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do Wait(0) end
