...
RegisterNetEvent("mc-admin/client/send-report", function(ReportData)
    if not HasReport() then
        ESX.ShowNotification(Lang:t('info.report_sent'))
        Config.Reports[#Config.Reports + 1] = ReportData
        TriggerServerEvent('mc-admin/server/sync-chat-data', 'Reports', Config.Reports, 1500)
        ToggleMenu(true)
    else
        ESX.ShowNotification(Lang:t('info.report_already', { chatcommand = Config.Commands['ReportChat'], chatcommandclose = Config.Commands['ReportClose'] }))
    end
end)
...
RegisterNetEvent("mc-admin/client/close-report", function()
    if HasReport() then
        ...
        if ReportId then
            ...
            if Success then
                ESX.ShowNotification(Lang:t('info.report_closed_self'))
            end
        end
    else
        ESX.ShowNotification(Lang:t('info.report_not', { chatcommand = Config.Commands['ReportNew'] }))
    end
end)
...
RegisterNetEvent("mc-admin/client/reply-report", function(Message, Time)
    if HasReport() then
        ...
        if ReportId then
            ...
            if Success then
                ESX.ShowNotification(Lang:t('info.report_reply_success'))
            else
                ESX.ShowNotification(Lang:t('info.report_reply_error'))
            end
        end
    else
        ESX.ShowNotification(Lang:t('info.report_not', { chatcommand = Config.Commands['ReportNew'] }))
