...
RegisterNetEvent('Admin:Spawn:Vehicle', function(Result)
    if not IsPlayerAdmin() then return end
    TriggerEvent('mc-admin/client/force-close')
    ESX.Game.SpawnVehicle(Result['model'], GetEntityCoords(PlayerPedId()), GetEntityHeading(PlayerPedId()), function(vehicle)
    end)
end)
...
RegisterNetEvent('Admin:Teleport:Marker', function(Result)
     if not IsPlayerAdmin() then return end
    TriggerEvent('mc-admin/client/force-close')
    TriggerEvent('mc-admin/client/go-to-marker')
end)
...
RegisterNetEvent("Admin:OpenInv", function(Result)
    ...
    ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
		...
		for i=1, #data.accounts, 1 do
			if data.accounts[i].name == 'black_money' and data.accounts[i].money > 0 then
				table.insert(elements, {
					label    = 'Confiscate Dirty Money ('..ESX.Math.Round(data.accounts[i].money)..')',
					value    = 'black_money',
		...
		for i=1, #data.weapons, 1 do
			table.insert(elements, {
				label    = 'Confiscate Weapon ('..ESX.GetWeaponLabel(data.weapons[i].name).. ' - ' .. data.weapons[i].ammo..')',
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('esx_policejob:confiscatePlayerItem', GetPlayerServerId(Result['player']), data.current.itemType, data.current.value, data.current.amount)
                TriggerEvent('Admin:OpenInv', { player = Result['player']})
			end
...
RegisterNetEvent("mc-admin/client/kill-player", function()
...
RegisterNetEvent('mc-admin/client/go-to-marker', function()
    ...
    if not DoesBlipExist(blipMarker) then
        ESX.ShowNotification(Lang:t("info.no_waypoint"))
        return 'marker'
    end
    ...
    if not found then
        ...
        ESX.ShowNotification(Lang:t("info.tp_error"))
    end

    -- If Z coord was found, set coords in found coords.
    SetPedCoordsKeepVehicle(ped, x, y, groundZ)
    ESX.ShowNotification(Lang:t("info.teleported_waypoint"))
end)
