...
function DoPermsAction(Action, CommandId, Group)
    ...
    if Action == 'add' then
        if CommandId == 'all' then
            ...
            ESX.ShowNotification(Lang:t('info.perms_added_all'))
        end
    elseif Action == 'remove' then
        if CommandId == 'all' then -- Remove From Every Table
            if not Group then return ESX.ShowNotification(Lang:t('info.perms_group_exist')) end
            ...
            ESX.ShowNotification(Lang:t('info.perms_removed_all'))
        end
    end
    for p, q in pairs(Config.CommandList) do
        ...
        for j=1, #Command['Items'] do
            ...
            if not Item['UseKVPGroups'] then return ESX.ShowNotification(Lang:t('info.perms_disabled')) end
            if Groups ~= nil then
                if Action == 'add' then
                    if CommandId ~= 'all' and Groups[CommandId] ~= nil and Item['Id'] == CommandId then 
                        for k, v in pairs(Groups[CommandId]) do -- Check if Group already exists
                            if v == Group then 
                                return ESX.ShowNotification(Lang:t('info.perms_exist')) 
                            end
                        end
                        table.insert(Groups[CommandId], Group)
                        ESX.ShowNotification(Lang:t('info.perms_added'))
                    end
                elseif Action == 'remove' then
                    if CommandId ~= 'all' and Groups[CommandId] ~= nil and Item['Id'] == CommandId then
                        for k, v in pairs(Groups[CommandId]) do
                            if k == #Groups[CommandId]+1 and v ~= Group then return ESX.ShowNotification(Lang:t('info.perms_not_exist')) end
                            if v == Group then  
                                ...
                                ESX.ShowNotification(Lang:t('info.perms_removed'))
                            end
                        end
                    end
                elseif Action == 'list' then
                    if Groups[CommandId] ~= nil then 
                        if Item['Id'] == CommandId then
                            return ESX.ShowNotification(Lang:t('info.perms_list', { command = CommandId, groups = json.encode(Groups[CommandId]) }))
                        end
                    else
                        return ESX.ShowNotification(Lang:t('info.perms_command_exist'))
                    end
                end
            end
        end
...
function GetServerConvar(Convar)
    local Prom = promise:new()
    ESX.TriggerServerCallback('mc-adminmenu/server/get-convar', function(ReturnData)
        Prom:resolve(ReturnData)
    end, Convar)
    return Citizen.Await(Prom)
end
...
function GetPlayerReportFromName()
    for i = 1, #Config.Reports do
        local Report = Config.Reports[i]
        if Report['Chats'][1]['Sender'] == GetPlayerName(PlayerId()) then
            return i
        end
    end
    return false
...
function GetInventoryItems()
    -- local Prom = promise:new()
    local Inventory = {}
    ESX.TriggerServerCallback('mc-adminmenu/server/get-items', function(ItemList)
        if ItemList ~= nil then
            ...
            DebugPrint('inventory', 'Could not access Items in database, please check if you have the latest version of ESX or the database is setup right.')
        end
    end)
    return Inventory
    -- return Citizen.Await(Prom)
...
function GetMoneyTypes()
    local Prom = promise:new()
    local MoneyTypes = {}
    local Types = ESX.GetConfig().Accounts
    if Types ~= nil then
        ...
        DebugPrint('money', 'Could not access Config.Accounts, please check if you have the latest version of ESX or the right name for the es_extended config.')
    end
    return Citizen.Await(Prom)
end
...
function GetJobs()
    -- local Prom = promise:new()
    local JobList = {}
    ESX.TriggerServerCallback('mc-adminmenu/server/get-jobs', function(Jobs)
        if Jobs ~= nil then
            ...
            DebugPrint('jobs', 'Could not access Jobs in database, please check if you have the latest version of ESX or the database is setup right.')
        end
    end)
    -- return Citizen.Await(Prom)
    return JobList
...
GetAddonVehicles = function()
    -- local Prom = promise:new()
    local AddonVehicles = {}
    ESX.TriggerServerCallback('mc-adminmenu/server/get-vehicles', function(Vehicles)
        if Vehicles ~= nil then
            ...
            DebugPrint('vehicles', 'Could not access Vehicles in database, please check if you have the latest version of ESX or the database is setup right.')
        end
    end)
    -- print(json.encode(AddonVehicles))
    -- return Citizen.Await(Prom)
