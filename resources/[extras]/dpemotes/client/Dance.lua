...
RegisterCommand('dance', function(source, args)
    local Dancenumber = tonumber(args[1]) or -1
    TriggerEvent('Ttyy-emotes:dance', Dancenumber)
end)

RegisterNetEvent('Ttyy-emotes:dance')
AddEventHandler('Ttyy-emotes:dance', function(dance)
    if not isHandcuffed and not isSoftCuffed then
        local noAnimations = #animations

        if dance == -1 then
            dance = math.random(noAnimations)
            print(("Random: dance number %s"):format(dance))
        end

        if dance > noAnimations or dance <= 0 then
            TriggerEvent('SendAlert', ("There are only %s dances, select a number inbetween or random (no input)"):format(noAnimations))
            return
        end 

        if animations[dance] and animations[dance].disabled then
            TriggerEvent('SendAlert', "This dance is disabled")
            return
        end

        LoadAnimationDic(animations[dance].dict)
        TaskPlayAnim(PlayerPedId(), animations[dance].dict, animations[dance].anim, 3.0, 3.0, -1, 1, 0, 0, 0, 0)
    end
end)
