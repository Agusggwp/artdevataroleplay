...
local function percentToInt(percentValue, cb)
	local totalValue = 1000000
	local calculate = (percentValue/100) * totalValue
	-- local calculate = (percentValue*totalValue)/100
	...
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				notification("~g~Mobil sudah diperbaki !!            🤩 VIP SYSTEM 🤩")
				...
				SetVehicleEngineOn(vehicle, true, false )
				SetVehicleOilLevel(vehicle,(GetVehicleOilLevel(vehicle)/3)-0.5)
				notification("~g~Mobil sudah diperbaki !!            🤩 VIP SYSTEM 🤩")			end
		else
			    SetVehicleUndriveable(vehicle,false)
				SetVehicleFixed(vehicle)
				SetVehicleDeformationFixed(vehicle)
...
RegisterNetEvent('iens:clean')
AddEventHandler('iens:clean', function()
	if isPedDrivingAVehicle() then
		local ped = PlayerPedId()
		vehicle = GetVehiclePedIsIn(ped, false)
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				notification("~g~Mobil kamu telah dibersihkan!             🤩 VIP SYSTEM 🤩")
			end
		else
				SetVehicleDirtLevel(vehicle, 0)
				notification("~g~Mobil kamu telah dibersihkan!             🤩 VIP SYSTEM 🤩")
...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			if cfg.preventVehicleFlip then
				...
				if (roll > 75.0 or roll < -75.0) and GetEntitySpeed(vehicle) < 2 then
					-- print('kebalik '.. addStressAttempt)
		...
		-- CAR ACCIDENT EFFECT
		local kendaraan = GetVehiclePedIsIn(PlayerPedId(), false)
			
		if DoesEntityExist(kendaraan) and (wasInCar or IsCar(kendaraan)) then
			letSleep = false
			wasInCar = true
			oldSpeed = currentSpeed
			oldBodyDamage = currentDamage
			currentDamage = GetVehicleBodyHealth(kendaraan)
			currentSpeed = GetEntitySpeed(kendaraan) * 2.23
			...
			if currentDamage ~= oldBodyDamage then
				...
				if currentDamage < oldBodyDamage then
					...
					if oldBodyDamage - currentDamage >= 17 then
						nabrakCount = nabrakCount + 1
						if nabrakCount >= 7 then
...
RegisterNetEvent('vip:repair')
AddEventHandler('vip:repair', function()
    local ped = PlayerPedId()
