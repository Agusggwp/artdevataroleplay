...
RegisterNetEvent('ttyy_tireslash:slash', function()
            ...
            if closestTire ~= nil then
                if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
                    ...
                    if driverServId == 0 then
                        ...
                        TriggerServerEvent('ttyy_tireslash:sync', driverServId, closestTire.tireIndex)
                    end
                    Wait((animDuration / 2) * 1000)
                    ClearPedTasks(PlayerPedId())
                    RemoveAnimDict(animDict)
                else
                    TriggerEvent('ttyy_tireslash:notify', Language['already_slashed'])
                end
            end
        else
            TriggerEvent('ttyy_tireslash:notify', Language['no_weapon'])
        end
    end
end)

RegisterNetEvent('ttyy_tireslash:sync')
AddEventHandler('ttyy_tireslash:sync', function(tireIndex)
	TriggerEvent('ttyy_tireslash:notify', Language['car_slashed'])
