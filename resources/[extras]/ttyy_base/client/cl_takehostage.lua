...
function menutakehostage()
	...
	if not holdingHostageInProgress and canTakeHostage then
		...
		if closestPlayer ~= nil and not IsPedDeadOrDying(targetPed) then
			print('allowed')
...
RegisterCommand("takehostage",function(source, args)
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		-- TODO : THE THREAD HOLDINGHOSTAGE
...
AddEventHandler('cmg3_animations:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach)
	...
	if animation2 == "victim_fail" then 
		...
		people1 = NetworkGetPlayerIndexFromPed(targetPed)
		people2 = NetworkGetPlayerIndexFromPed(playerPed)
		TriggerServerEvent('DiscordBot:playerDied', GetPlayerName(people1) .. ' ' .. ' kill the hostage name ' .. GetPlayerName(people2).. ' .', 'Pistol')
...
function GetClosestPlayer(radius)
	...
	if closestDistance <= 1.0 then -- ucok changes this radius to 1.0
