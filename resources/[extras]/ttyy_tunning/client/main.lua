...
AddEventHandler('esx_lscustom:installMod', function()
	...
	TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 0.1, 'repairmodif', 0.1)
	...
	for k,v in pairs(Config.Menus) do
		...
		if data.value == k then
			...
			if v.modType then
				...
				elseif v.modType == 'extras' then -- Extras
					local _label = ''
					price = math.floor(vehiclePrice * v.price / 1000)
					for i=1, 14, 1 do
						local installed = (currentMods.extras[tostring(i)] ~= nil and currentMods.extras[tostring(i)] == true)
						local textPrice = ' - <span style="color:green;">$ ' .. price
						if installed then
							textPrice = ' - <span style="color:cornflowerblue;">$ '.. price
						end
						if DoesExtraExist(vehicle, i) then
							table.insert(elements, {
								label = '<span style="color:green">Extra #' .. i .. textPrice .. '</span>',
								modType = k,
								modNum = i,
				...
				elseif v.modType == 17 then -- TURBO
					...
					if currentMods[k] then
						...
						_label = 'Turbo - <span style="color:green;">$' .. math.floor(vehiclePrice * v.price[1] / 1000) .. ' </span>'
					end
					table.insert(elements, {label = _label, modType = k, modNum = true})
				else
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (PlayerData.job and PlayerData.job.name == 'mechanic' and PlayerData.job.grade > 1) or (PlayerData.job and PlayerData.job.name == 'speedhunter' and PlayerData.job.grade > 1)  or not Config.IsMechanicJobOnly then
				for k,v in pairs(Config.Zones) do
