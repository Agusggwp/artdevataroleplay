...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if inRange and not shown then
        	shown = true
        	--exports['ttyy_nuipack']:Open('[E] To access the bank', 'darkblue', 'left') 
...
Citizen.CreateThread(function()
	...
	exports["qtarget"]:AddTargetModel(bank, {
		options = {
		{
			event = "okokBanking:openmenu",
...
Citizen.CreateThread(function()
    ...
    while true do
            ...
            if IsControlJustReleased(0, 38) then
            	ESX.TriggerServerCallback("okokBanking:GetPIN", function(pin)
            		if pin then
						...
						exports['ttyy_nuipack']:Alert("BANK", "Head up to a bank to set a PIN code", 5000, 'info')
					end
				end)
            end
        ...
        if inRange and not shown then
        	shown = true
        	exports['ttyy_nuipack']:Open('[E] To access the ATM', 'darkblue', 'left') 
        elseif not inRange and shown then
        	shown = false
        	exports['ttyy_nuipack']:Close()
        end
	...
	elseif data.action == "deposit" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				...
				exports['ttyy_nuipack']:Alert("BANK", "Invalid amount", 5000, 'error')
			end
		else
			exports['ttyy_nuipack']:Alert("BANK", "Invalid input", 5000, 'error')
		end
			...
			if tonumber(data.value) > 0 then
				ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
					...
					elseif isUsed == nil then
						exports['ttyy_nuipack']:Alert("BANK", "This IBAN does not exist", 5000, 'error')
					end
				end, data.iban:upper())
			else
				exports['ttyy_nuipack']:Alert("BANK", "Invalid amount", 5000, 'error')
	...
	elseif data.action == "change_iban" then
		if Config.CustomIBANAllowLetters then
			...
			ESX.TriggerServerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
				...
				elseif isUsed ~= nil then
					exports['ttyy_nuipack']:Alert("BANK", "This IBAN is already in use", 5000, 'error')
				end
			end, iban)
		elseif not Config.CustomIBANAllowLetters then
			if tonumber(data.iban) ~= nil then
				...
				exports['ttyy_nuipack']:Alert("BANK", "You can only use numbers on your IBAN", 5000, 'error')
			end
		end
	elseif data.action == "change_pin" then
		if tonumber(data.pin) ~= nil then
			if string.len(data.pin) == 4 then
				...
				exports['ttyy_nuipack']:Alert("BANK", "Your PIN needs to be 4 digits long", 5000, 'info')
			end
		else
			exports['ttyy_nuipack']:Alert("BANK", "You can only use numbers", 5000, 'info')
		end
	end
end)
...
RegisterNetEvent("okokBanking:openmenu")
AddEventHandler("okokBanking:openmenu", function()
	SetNuiFocus(true, true)
	SendNUIMessage({
		action = 'loading_data',
	})
