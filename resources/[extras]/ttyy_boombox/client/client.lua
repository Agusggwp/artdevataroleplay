...
RegisterNetEvent('ttyy_boombox:useBoombox')
AddEventHandler('ttyy_boombox:useBoombox', function()
...
RegisterNetEvent('ttyy_boombox:deleteObj', function(netId)
    if DoesEntityExist(NetToObj(netId)) then
        DeleteObject(NetToObj(netId))
        if not DoesEntityExist(NetToObj(netId)) then
            TriggerServerEvent('ttyy_boombox:objDeleted')
        end
    end   
end)

AddEventHandler('ttyy_boombox:pickup', function()
    local ped = PlayerPedId()
    ...
    if xSound:soundExists(musicId) then
        TriggerServerEvent("ttyy_boombox:soundStatus", "stop", musicId, {})
    end
    FreezeEntityPosition(closestRadio, false)
    TriggerServerEvent("ttyy_boombox:deleteObj", ObjToNet(closestRadio))
    if activeRadios[closestRadio] then
        activeRadios[closestRadio] = nil
    end 
    TriggerServerEvent('ttyy_boombox:syncActive', activeRadios)
    ClearPedTasks(ped)
end)
...
AddEventHandler('ttyy_boombox:interact', function()
...
AddEventHandler('ttyy_boombox:savedSongs', function(radio)
    savedSongsMenu(radio)
end)

AddEventHandler('ttyy_boombox:saveSong', function()
    local input = lib.inputDialog('Save Song', {'Name', 'Youtube Link'})
    if input[1] and input[2] then
        TriggerServerEvent('ttyy_boombox:save', input[1], input[2])
        lib.notify({
            title = 'Success',
...
AddEventHandler('ttyy_boombox:selectSavedSong', function(data)
    selectSavedSong(data)
end)

AddEventHandler('ttyy_boombox:playSavedSong', function(data)
    local musicId = 'id_'..data.id
    TriggerServerEvent("ttyy_boombox:soundStatus", "play", musicId, { position = activeRadios[data.id].pos, link = data.link, volume = '0.2', distance = 7 })
...
AddEventHandler('ttyy_boombox:deleteSong', function(data)
	...
	if confirmed == 'confirm' then
		TriggerServerEvent('ttyy_boombox:deleteSong', data)
		lib.notify({
			title = 'Deleted',
...
AddEventHandler('ttyy_boombox:playMenu', function(data)
    local musicId = 'id_'..data.id
    if data.type == 'play' then
        ...
        if keyboard then
            if keyboard[1] and tonumber(keyboard[2]) and tonumber(keyboard[2]) <= 40 and tonumber(keyboard[3]) and tonumber(keyboard[3]) <= 100 then
                TriggerServerEvent("ttyy_boombox:soundStatus", "play", musicId, { position = activeRadios[data.id].pos, link = keyboard[1], volume = keyboard[3]/100, distance = keyboard[2] })
                activeRadios[data.id].data = {playing = true, currentId = 'id_'..PlayerId()}
                TriggerServerEvent('ttyy_boombox:syncActive', activeRadios)
            end
        end
    ...
    elseif data.type == 'volume' then
        ...
        if keyboard then
            if tonumber(keyboard[1]) and tonumber(keyboard[1]) <= 100 then
                TriggerServerEvent("ttyy_boombox:soundStatus", "volume", musicId, {volume = keyboard[1]/100})
            end
        end
    elseif data.type == 'distance' then
        ...
        if keyboard then
            if tonumber(keyboard[1]) and tonumber(keyboard[1]) <= 40 then
                TriggerServerEvent("ttyy_boombox:soundStatus", "distance", musicId, {distance = keyboard[1]})
            end
        end
    end
end)
...
AddEventHandler('ttyy_boombox:syncActive', function(activeBoxes)
