...
function SocietyInvoices(society)
	ESX.TriggerServerCallback("okokBilling:GetSocietyInvoices", function(cb, totalInvoices, totalIncome, totalUnpaid, awaitedIncome)
		if json.encode(cb) ~= '[]' then
			...
			exports['ttyy_nuipack']:Alert("BILLING", "Your society doesn't have invoices.", 10000, 'info')
...
RegisterNetEvent("ttyy:invoices")
AddEventHandler("ttyy:invoices", function()
	MenuInvoices()
end)

function MenuInvoices()
...
RegisterNUICallback("action", function(data, cb)
	...
	elseif data.action == "createInvoice" then
		...
		if closestPlayer == -1 or playerDistance > 3.0 then
			exports['ttyy_nuipack']:Alert("BILLING", "Error sending the invoice! There's nobody near you.", 10000, 'error')
		else
			TriggerServerEvent("okokBilling:CreateInvoice", data)
			exports['ttyy_nuipack']:Alert("BILLING", "Invoice successfully sent!", 10000, 'success')
		end
		
		SetNuiFocus(false, false)
	elseif data.action == "missingInfo" then
		exports['ttyy_nuipack']:Alert("BILLING", "Fill in all fields before sending an invoice!", 10000, 'error')
	elseif data.action == "negativeAmount" then
		exports['ttyy_nuipack']:Alert("BILLING", "You need to set a positive amount!", 10000, 'error')
		...
		for k, v in pairs(Config.AllowedSocieties) do
			if v == PlayerData.job.name then
				...
				elseif Config.OnlyBossCanAccessSocietyInvoices then
					exports['ttyy_nuipack']:Alert("BILLING", "Only the boss can access the society invoices.", 10000, 'error')
				end
			end
		end
