...
local function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            ESX.ShowNotification('You cant go to fast')
        end
    else
        ESX.ShowNotification('You cant go too fast')
    end
end
...
local function SaveRace()
    ...
    TriggerServerEvent('logi_lapraces:server:SaveRace', CreatorData)

    ESX.ShowNotification('Race: '..CreatorData.RaceName..' is saved!')
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, 162) or IsDisabledControlJustPressed(0, 162) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        ESX.ShowNotification('You have not placed any checkpoints yet..')
                    end
                end
                ...
                if IsControlJustPressed(0, 311) or IsDisabledControlJustPressed(0, 311) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        ESX.ShowNotification('You must have at least 10 checkpoints')
                    end
                end
            ...
            if IsControlJustPressed(0, 163) or IsDisabledControlJustPressed(0, 163) then
                if not CreatorData.ConfirmDelete then
                    ...
                    ESX.ShowNotification('Press [9] again to confirm')
                else
                    ...
                    CreatorData.RaceName = nil
                    CreatorData.Checkpoints = {}
                    ESX.ShowNotification('Race-editor canceled!')
...
local function FinishRace()
    TriggerServerEvent('logi_lapraces:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
...
RegisterNetEvent('logi_lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        ESX.ShowNotification('You are already making a race.')
    end
end)

RegisterNetEvent('logi_lapraces:client:UpdateRaceRacerData', function(RaceId, aRaceData)
    if (CurrentRaceData.RaceId ~= nil) and CurrentRaceData.RaceId == RaceId then
        CurrentRaceData.Racers = aRaceData.Racers
    end
end)

RegisterNetEvent('logi_lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('logi_lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        ESX.ShowNotification('Youre already in a race..')
    end
end)

RegisterNetEvent('logi_lapraces:client:LeaveRace', function(_)
...
RegisterNetEvent('logi_lapraces:client:RaceCountdown', function()
    TriggerServerEvent('logi_lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    ESX.ShowNotification('The race will start in 10 seconds')
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    ESX.ShowNotification(Countdown)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            ...
            ESX.ShowNotification('GO!', 'success')
        ...
        ESX.ShowNotification('You are not currently in a race..')
    end
end)

RegisterNetEvent('logi_lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            ESX.ShowNotification(FinisherData..' is finished on spot: '..Place)
        end
    end
end)

RegisterNetEvent('logi_lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            ESX.ShowNotification('Go back to the start or you will be kicked from the race: '..ToFarCountdown..'s')
                        else
                            TriggerServerEvent('logi_lapraces:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Wait(1000)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('logi_lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('logi_lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
