...
RegisterServerEvent("ttyy_dealerships:playerEnteredDealership")
AddEventHandler("ttyy_dealerships:playerEnteredDealership", function(dealership)
    table.insert(playersInDealerships,  {dealership = dealership, player = source})
end)

RegisterServerEvent("ttyy_dealerships:playerLeftDealership")
AddEventHandler("ttyy_dealerships:playerLeftDealership", function()
...
RegisterServerEvent("ttyy_dealerships:updateDisplays")
AddEventHandler("ttyy_dealerships:updateDisplays", function(display)
    for k, v in pairs(playersInDealerships) do
        if v.player ~= source then
            TriggerClientEvent('ttyy_dealerships:updateTable', v.player, display)
        end
    end
end)

RegisterServerEvent("ttyy_dealerships:getDisplays")
AddEventHandler("ttyy_dealerships:getDisplays", function(dealership)
    ...
    TriggerClientEvent('ttyy_dealerships:sendDisplays', source, dealearshipDisplays)
end)

RegisterServerEvent("ttyy_dealerships:removeDisplayVehicle")
AddEventHandler("ttyy_dealerships:removeDisplayVehicle", function(dealership, display)
        ...
        if v.zone.name == dealership then
            for k2, v2 in pairs(Displays) do
                if v2.display == display then
                    TriggerEvent('ttyy_dealerships:updateDisplays', display)
...
RegisterServerEvent("ttyy_dealerships:setDisplayVehicle")
AddEventHandler("ttyy_dealerships:setDisplayVehicle", function(data)
...
RegisterServerEvent("ttyy_dealerships:getDealershipVehicles")
AddEventHandler("ttyy_dealerships:getDealershipVehicles", function(dealership, display)
    ...
    TriggerClientEvent('ttyy_dealerships:viewDealershipVehicles', source, purchasableVehicles, display)
end)


RegisterServerEvent("ttyy_dealerships:confirmCustomerPurchase")
AddEventHandler("ttyy_dealerships:confirmCustomerPurchase", function(args, player, financed, financeLength, downPayment)
    TriggerClientEvent('ttyy_dealerships:confirmPurchase', player, args, nil, financed, financeLength, downPayment, true, source)
end)

RegisterServerEvent("ttyy_dealerships:checkPeopleWorking")
AddEventHandler("ttyy_dealerships:checkPeopleWorking", function(job, fromDealer)
    ...
    if playersWorking == true and job ~= nil then
        if xPlayer.job.name == job or fromDealer then
            TriggerClientEvent('ttyy_dealerships:showMenu', source, "purchase_confirmation_menu", true)
        else 
			TriggerClientEvent("ttyy_dealerships:notification",source , "Go speak to a dealer", nil, "inform")
        end
    else
        TriggerClientEvent('ttyy_dealerships:showMenu', source, "purchase_confirmation_menu", true)
    end
...
ESX.RegisterServerCallback('ttyy_dealerships:isPlateTaken', function(source, cb, plate)
...
ESX.RegisterServerCallback('ttyy_dealerships:buyVehicle', function(source, cb, model, plate, vehicleType, dealership, label, dealerID)
    local xPlayer = ESX.GetPlayerFromId(source)
    local Players = ESX.GetPlayers()
    local playersWorking = false
    for i = 1, #Players, 1 do
    ...
    if price and balance >= price then
		MySQL.update('INSERT INTO owned_vehicles (owner, plate, vehicle, type) VALUES (@owner, @plate, @vehicle, @type)', {
			...
			TriggerClientEvent("ttyy_dealerships:notification",source , "A "..label.." now belongs to you","Plate: "..plate, "success")
			cb(true)
		end)
	else
		cb(false)
...
ESX.RegisterServerCallback('ttyy_dealerships:dealearCheck', function(source, cb, job)
	local xPlayer = ESX.GetPlayerFromId(source)
    local Players = ESX.GetPlayers()
    local playersWorking = false
    for i = 1, #Players, 1 do
...
RegisterServerEvent("ttyy_dealerships:getFinancedVehicles")
AddEventHandler("ttyy_dealerships:getFinancedVehicles", function()
	local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if financedVehicles or repoedVehicles then
        TriggerClientEvent('ttyy_dealerships:finaceCheckMenu', _source, financedVehicles, repoedVehicles)
    end
end)

ESX.RegisterServerCallback('ttyy_dealerships:financeVehicle', function(source, cb, model, plate, vehicleType, financeLength, downPayment, label, dealerID)
	local xPlayer = ESX.GetPlayerFromId(source)
    local price = Config.AllVehicles[model].price
    local balance
...
RegisterServerEvent("ttyy_dealerships:makePayment")
AddEventHandler("ttyy_dealerships:makePayment", function(plate, amount, owner, vehicleModel, payFull)
    local _source = source
    ...
    if amount <= balance then
        ...
        if paymentsLeft == 0 or payFull then
            ...
            TriggerClientEvent("ttyy_dealerships:notification",_source, "Payment made", "You've fully paid off your "..vehicleModel, "success")
            if Config.usePEFCL then
            ...
            TriggerClientEvent("ttyy_dealerships:notification",_source, "Payment made", paymentsLeft.." payments remaining", "success")
            if Config.usePEFCL then
        ...
        TriggerClientEvent("ttyy_dealerships:notification",_source, "Not enough money", nil, "error")
    end
end)

AddEventHandler('onServerResourceStart', function(resourceName)
    if resourceName == 'ttyy_dealerships' or resourceName == GetCurrentResourceName() then
                ...
                for i=1, #vehicles, 1 do
                    ...
                    if todaysDate > vehicles[i].nextpayment and vehicles[i].financed == 1 then
                        ...
                        elseif vehicles[i].paymentsoverdue > Config.maxMissedPayments then
                            TriggerEvent("ttyy_dealerships:repoVehicle", vehicles[i].plate)
                        else
                            local t = os.time()
                            local date = os.date("%Y%m%d",t)
                            local d = 7
...
RegisterServerEvent("ttyy_dealerships:repoVehicle")
AddEventHandler("ttyy_dealerships:repoVehicle", function(plate)
                ...
                if changed then
                    MySQL.update('DELETE FROM owned_vehicles WHERE plate = @plate', {['@plate'] = plate}, function(affectedRows)
                        if affectedRows then
                            ...
                            TriggerClientEvent("ttyy_dealerships:notification",source, "Something went wrong", nil, "error")
                        end
                    end)
                else
                    TriggerClientEvent("ttyy_dealerships:notification",source, "Something went wrong", nil, "error")
...
RegisterServerEvent("ttyy_dealerships:recoverRepoedVehicle")
AddEventHandler("ttyy_dealerships:recoverRepoedVehicle", function(plate, vehicleModel)
    local _source = source
                ...
                MySQL.insert('INSERT INTO owned_vehicles (owner, plate, vehicle, type, job, weeklypayment) VALUES (?, ?, ?, ?, ?, ?)', {row.owner, row.plate, row.vehicle, row.type, row.job, row.weeklypayment}, function(changed)
                    if changed then
                        ...
                        MySQL.update('DELETE FROM repoed_vehicles WHERE plate = @plate', {['@plate'] = row.plate}, function(affectedRows)
                            if affectedRows then
                                TriggerClientEvent("ttyy_dealerships:notification",_source, "Successfully recovered "..vehicleModel.." from repo", "Plate: "..row.plate, "success")
                                if Config.usePEFCL then
                                ...
                                TriggerClientEvent("ttyy_dealerships:notification",_source, "Something went wrong", nil, "error")
                            end
                        end)
                    else
                        TriggerClientEvent("ttyy_dealerships:notification",_source, "Something went wrong", nil, "error")
...
RegisterServerEvent("ttyy_dealerships:abandonRepoedVehicle")
AddEventHandler("ttyy_dealerships:abandonRepoedVehicle", function(plate)
    local _source = source
    MySQL.update('DELETE FROM repoed_vehicles WHERE plate = @plate', {['@plate'] = plate}, function(affectedRows)
        if affectedRows then
            TriggerClientEvent("ttyy_dealerships:notification",_source, "Successfully abandoned vehicle", "Plate: "..plate, "success")
        else
            TriggerClientEvent("ttyy_dealerships:notification",_source, "Something went wrong", nil, "error")
        end
    end)
...
ESX.RegisterServerCallback('ttyy_dealerships:requestPlayerCars', function(source, cb, plate)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterServerEvent('ttyy_dealerships:setVehicleOwnedPlayerId')
AddEventHandler('ttyy_dealerships:setVehicleOwnedPlayerId', function (playerId, vehicleProps)
