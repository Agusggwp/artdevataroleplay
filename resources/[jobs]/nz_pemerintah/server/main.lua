	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_pemerintah', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			...
			if xPlayer.canCarryItem(itemName, count) then
                ...
                TriggerClientEvent('ttyy_nuipack:client:SendAlert', xPlayer.source, { type = 'success', text = 'Berhasil mengambil ' ..inventoryItem.label.. ' sejumlah ' ..count.. ' dari berangkas'})
                ...
                TriggerClientEvent('ttyy_nuipack:client:SendAlert', xPlayer.source, { type = 'error', text = 'Jumlah tidak valid!'})
			end
		else
            TriggerClientEvent('ttyy_nuipack:client:SendAlert', xPlayer.source, { type = 'error', text = 'Jumlah tidak valid!'})
			-- xPlayer.showNotification(_U('quantity_invalid'))
...
AddEventHandler('nz_pemerintah:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_pemerintah', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
            ...
            TriggerClientEvent('ttyy_nuipack:client:SendAlert', xPlayer.source, { type = 'success', text = 'Berhasil deposit ' ..inventoryItem.label.. ' sejumlah ' ..count})
