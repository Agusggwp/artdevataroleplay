...
AddEventHandler('esx_plasticsurgery:pay', function()
	...
	xPlayer.removeMoney(ConfigOprasi.Price)

	TriggerClientEvent('esx:showNotification', source, _U('you_paid') .. '$' .. ConfigOprasi.Price)
...
ESX.RegisterServerCallback('esx_plasticsurgery:checkMoney', function(source, cb)
	...
	if xPlayer.get('money') >= ConfigOprasi.Price then
		cb(true)
	else
		cb(false)
	end
