if Cfg_Pdg.MaxInService ~= -1 then
  TriggerEvent('esx_service:activateService', 'pedagang', Cfg_Pdg.MaxInService)
end
...
RegisterServerEvent("ttyy-bottle:retrieveBottle")
AddEventHandler("ttyy-bottle:retrieveBottle", function()
    local player = ESX.GetPlayerFromId(source)

    math.randomseed(os.time())
    local luck = math.random(0, 69)
    local randomBottle = math.random((1 or 4), (4 or 6))

    if luck >= 0 and luck <= 29 then
        TriggerClientEvent("mythic_notify:SendNotification", -1, {text = "There's Mothing Here.", type = "error", queue = "lmao", timeout = 3000, layout = "centerLeft"})
    else
        player.addInventoryItem("bottle", randomBottle)
        TriggerClientEvent("mythic_notify:SendNotification", -1, {text = ("You Got x%s Bottle"):format(randomBottle), type = "success", queue = "lmao", timeout = 3000, layout = "centerLeft"})
    end
end)
    ...
    if _itemValue == 'jagerbomb' then
        SetTimeout(10000, function()        
            ...
            elseif bethQuantity < 2 then
                ...
                if chanceToMiss <= Cfg_Pdg.MissCraft then
