    ...
    window.addEventListener('message', function (event) {
        ...
        if(event.data.foundPlayers){ 
            ...
            event.data.foundPlayers.forEach(function(user){
                ...
                $('<td class="repers_content">').append($('<button class="boton_lateral" id="lspd_vm_button" style="position:relative;">See more</button>').click(function(){
        ...
        if(event.data.otherLInfo){
            $('#lspd_perf_nom').text('Name: '+event.data.otherLInfo.name);
            $('#lspd_perf_emp').text('Job: '+event.data.otherLInfo.job.label+' - '+event.data.otherLInfo.job.grade_label);
            $('#lspd_perf_est').text('Height: '+event.data.otherLInfo.Height+' m');
            $('#lspd_perf_fecha').text('Birth Date: '+event.data.otherLInfo.fecha);
            $('#lspd_perf_tel').text('Phone: '+event.data.otherLInfo.phone);
            $("#lspd_plyFoto").attr("src", event.data.otherLInfo.foto);
            $("#lspd_perf_multas").text('Total Denda: '+ event.data.otherLInfo.sinPagar + ' $')
            if(event.data.otherLInfo.sex == 'm'){
                $('#lspd_perf_gen').text('Gender : Male');
            }else{
                $('#lspd_perf_gen').text('Gender : Female');
            }
        }
        ...
        if(event.data.multasList){
            ...
            event.data.multasList.forEach(function(list){
                ...
                $('#lspd_add_delito_content').append($('<tr>').append(
                    ...
                    $('<td class = "lspd_add_delito_content">').append($('<button class = "boton_sup">Modify</button>').click(function(){
                        $('#input_lspd_delitos_label').val(list['label']);
                        $('#input_lspd_delitos_price').val(list['price']);
                        $('#input_lspd_delitos_time').val(list['time']);
                    })
                    ),
                    $('<td class = "lspd_add_delito_content">').append($('<button style = "font-size:12px;" class = "boton_sup">Add Crime</button>').click(function(){
        ...
        if(event.data.delitosPerfil){ 
            ...
            event.data.delitosPerfil.forEach(function(del){
                ...
                $('<td class="delitos_content">').text(del['time'] + ' months')));
            })
        }
        ...
        if(event.data.delitosPersona){
            ...
            event.data.delitosPersona.forEach(function(delitos){
                $('#lspd_hist_content').append($('<tr>').append(
                    ...
                    $('<td class = "lspd_hist_content">').text(delitos['time']+' months'),
                    $('<td class = "lspd_hist_content">').append('<button class = "boton_sup">Remove</button>').click(function(){
        ...
        if(event.data.notasPersona){
            ...
            event.data.notasPersona.forEach(function(notas){
                $('#lspd_notas_content').append($('<tr>').append(
                    ...
                    $('<td class = "lspd_notas_content">').append('<button class = "boton_sup">Remove</button>').click(function(){
        ...
        if(event.data.busquedaPersona){
            if(event.data.busquedaPersona.inSearch == true){
                ...
                $('#lspd_perf_search').text('Subject in Search and Capture: YES');
                $('#boton_editar_busca').fadeIn();
                $('#boton_poner_en_busca').hide();
            }else if(event.data.busquedaPersona.inSearch == false){
                $('#lspd_perf_search').text('Subject Wanted and Captured: No');
            ...
            event.data.allBusquedas.forEach(function(info){
                $('#lspd_buscaycap').append(
                    $('<div class = "busqueda_container">').append(
                        ...
                        $('<br>'),
                        $('<button style="margin:10px;" class="boton_sup"><i class= "fas fa-id-card"></i> View profile</button>').click(function(){
        ...
        if(event.data.foundEMSPlayers){ 
            ...
            event.data.foundEMSPlayers.forEach(function(user){
                ...
                $('<td class="repers_content">').text(user['lastname']),
                $('<td class="repers_content">').text(user['dateofbirth']),
                $('<td class="repers_content">').append($('<button class="boton_lateral" id="ems_vm_button" style="position:relative;">See more</button>').click(function(){
        ...
        if(event.data.otherEMSLInfo){
            $('#ems_perf_nom').text('Name: '+event.data.otherEMSLInfo.name);
            $('#ems_perf_emp').text('Job: '+event.data.otherEMSLInfo.job.label+' - '+event.data.otherEMSLInfo.job.grade_label);
            $('#ems_perf_est').text('Height: '+event.data.otherEMSLInfo.Height +' m');
            $('#ems_perf_fecha').text('Birth Date: '+event.data.otherEMSLInfo.fecha);
            $('#ems_perf_tel').text('Phone: '+event.data.otherEMSLInfo.phone);
            $("#ems_plyFoto").attr("src", event.data.otherEMSLInfo.foto);
            if(event.data.otherEMSLInfo.sex == 'm'){
                $('#ems_perf_gen').text('Gender : Male');
            }else{
                $('#ems_perf_gen').text('Gender : Female');
            }
        }
        ...
        if(event.data.tratsList){
            ...
            event.data.tratsList.forEach(function(list){
                ...
                $('#ems_add_trat_content').append($('<tr>').append(
                    ...
                    $('<td class = "ems_add_trat_content">').append($('<button class = "boton_sup">Modify</button>').click(function(){
        ...
        if(event.data.histmedicoPersona){
            ...
            event.data.histmedicoPersona.forEach(function(delitos){
                $('#ems_hist_content').append($('<tr>').append(
                    ...
                    $('<td class = "ems_hist_content">').append('<button class = "boton_sup">Remove</button>').click(function(){
        ...
        if(event.data.notasEMSPersona){
            ...
            event.data.notasEMSPersona.forEach(function(notas){
                $('#ems_notes_content').append($('<tr>').append(
                    ...
                    $('<td class = "ems_notes_content">').append('<button class = "boton_sup">Remove</button>').click(function(){
        ...
        if(event.data.foundmecaPlayers){ 
            ...
            event.data.foundmecaPlayers.forEach(function(user){
                ...
                $('<td class="repers_content">').text(user['lastname']),
                $('<td class="repers_content">').text(user['dateofbirth']),
                $('<td class="repers_content">').append($('<button class="boton_lateral" id="meca_vm_button" style="position:relative;">See more</button>').click(function(){
        ...
        if(event.data.othermecaLInfo){
            $('#meca_perf_nom').text('Name: '+event.data.othermecaLInfo.name);
            $('#meca_perf_emp').text('Job: '+event.data.othermecaLInfo.job.label+' - '+event.data.othermecaLInfo.job.grade_label);
            $('#meca_perf_est').text('Height: '+event.data.othermecaLInfo.Height +' m');
            $('#meca_perf_fecha').text('Birth Date: '+event.data.othermecaLInfo.fecha);
            $('#meca_perf_tel').text('Phone: '+event.data.othermecaLInfo.phone);
            $("#meca_plyFoto").attr("src", event.data.othermecaLInfo.foto);
            if(event.data.othermecaLInfo.sex == 'm'){
                $('#meca_perf_gen').text('Gender : Male');
            }else{
                $('#meca_perf_gen').text('Gender : Female');
            }
        }
        ...
        if(event.data.preciosMecaList){
            ...
            event.data.preciosMecaList.forEach(function(list){
                ...
                $('#meca_add_trat_content').append($('<tr>').append(
                    ...
                    $('<td class = "meca_add_trat_content">').append($('<button class = "boton_sup">Modify</button>').click(function(){
        ...
        if(event.data.histmecanicoPersona){
            ...
            event.data.histmecanicoPersona.forEach(function(delitos){
                $('#meca_hist_content').append($('<tr>').append(
                    ...
                    $('<td class = "meca_hist_content">').append('<button class = "boton_sup">Remove</button>').click(function(){
                        var info = JSON.stringify({date:delitos['date'],label:delitos['label'],officer:delitos['doctor']});
        ...
        if(event.data.notasmecaPersona){
            ...
            event.data.notasmecaPersona.forEach(function(notas){
                $('#meca_notes_content').append($('<tr>').append(
                    ...
                    $('<td class = "meca_notes_content">').append('<button class = "boton_sup">Remove</button>').click(function(){
                        var info = JSON.stringify({date:notas['date'],label:notas['label'],sender:notas['sender']});
        ...
        $('#boton_perfil').click(function(){
            $('.app').hide();
            $('#app_perfil').fadeIn();
            $('#perf_nom').text('Name: '+event.data.playerInfo.name);
            $('#perf_emp').text('Job: '+event.data.playerInfo.job.label+' - '+event.data.playerInfo.job.grade_label);
            $('#perf_est').text('Height: '+event.data.playerInfo.Height+' m');
            $('#perf_fecha').text('Birth Date: '+event.data.playerInfo.fecha);
            $('#perf_tel').text('Phone: '+event.data.playerInfo.phone);
            if(event.data.playerInfo.sex == 'm'){
                $('#perf_gen').text('Gender : Male');
            }else{
                $('#perf_gen').text('Gender : Female');
            }
