...
PRX.RegisterServerCallback('prx_tablet:getPlayerInfo', function(source, cb)
    ...
    MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @identifier', {
		...
		for i=1, #result, 1 do
            ...
            Height =result[i].height,fecha=result[i].dateofbirth, foto = result[i].foto}
		end
...
PRX.RegisterServerCallback('prx_tablet:getOtherPlayerInfo', function(source, cb, id)
    ...
    MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @identifier', {
		...
		for i=1, #result, 1 do
            ...
            phone =result[i].phone_number, sex =result[i].sex,
            Height =result[i].height,fecha=result[i].dateofbirth, identifier = id, foto = result[i].foto, sinPagar = billMoney}
		end
...
AddEventHandler('prx_tablet:addDelito', function(identifier, label, price, time)
    ...
    MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)', {
      ...
      if xTarget ~= nil then  
        xTarget.showNotification('you have been fined for ~r~'..label)
      end
  end)
