...
function startRoute(route)
    ...
    if activeRouteLine == nil then
        exports['ttyy_nuipack']:SendAlert('inform', _U('route_selection_cancel'))
        return
    end
    ...
    exports['ttyy_nuipack']:SendAlert('inform', _U('route_assigned', _U(activeRouteLine.Name)))
    ...
    exports['ttyy_nuipack']:SendAlert('inform', _U('drive_to_first_marker', firstStopName))
...
function handleNormalStop()
    ...
    if playerDistanceFromCoords(currentStop) < Config.Markers.Size then
        ...
        if (isLastStop(stopNumber)) then
            ...
            Blips.StopAbortBlip()
            exports['ttyy_nuipack']:SendAlert('inform', _U('return_to_terminal'))
        else
            exports['ttyy_nuipack']:SendAlert('inform', _U('drive_to_next_marker', nextStopName))
            setUpNextStop()
...
function payForEachPedLoaded(numberOfPeds)
    if numberOfPeds > 0 then
        ...
        exports['ttyy_nuipack']:SendAlert('inform', _U('passengers_loaded', numberOfPeds, amountToPay))
