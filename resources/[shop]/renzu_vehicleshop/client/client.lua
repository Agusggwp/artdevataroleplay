...
Citizen.CreateThread(function()
    ...
    for k, v in pairs (VehicleShop) do
        if v.ShowBlip == false then return end
        ...
        AddTextComponentSubstringPlayerName("[PUB] Vehicle Shop: "..v.name.."")
...
AddEventHandler('esx_giveownedcar:spawnVehicle', function(model, plate)
    ...
    TriggerServerCallback_("renzu_vehicleshop:GenPlate",function(plate)
        local generatedPlate = not plate and tostringplate(plate) or string.upper(plate)

        ESX.TriggerServerCallback('renzu_vehiceshop:isPlateTaken', function (isPlateTaken)
            if not isPlateTaken then
                ESX.Game.SpawnVehicle(model, coords, 0.0, function(vehicle) --get vehicle info
                    if DoesEntityExist(vehicle) then
                        ...
                        if model == 'freecrawler' then
                            TriggerServerEvent('esx_giveownedcar:setVehicleReward', vehicleProps, model)
                            ESX.ShowNotification('Kamu Mendapatkan Kendaraan ' .. model .. 'Dengan Nomor Plat ' .. generatedPlate)
                        else
