...
ESX.RegisterServerCallback('renzu_vehiceshop:isPlateTaken', function(source, cb, vPlateS)
...
AddEventHandler('esx_giveownedcar:setVehicleReward', function (vehicleProps, vehiclename)
	...
	local category, name = 'suvs', 'freecrawler'

	if xPlayer then
		MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, stored, type, job, last_garage, garage) VALUES (@owner, @plate, @vehicle, @stored, @type, @job, @last_garage, @garage)', {
			['@owner'] = xPlayer.identifier,
