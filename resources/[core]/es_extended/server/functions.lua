...
ESX.CreatePhoneNumber = function(identifier)
    local UniqueFound = false
    local PhoneNumber = nil
    while not UniqueFound do
        PhoneNumber = math.random(100,999) .. math.random(1000000,9999999)
        local query = PhoneNumber
        local result = MySQL.prepare.await('SELECT COUNT(*) as count FROM `users` WHERE `phone_number` = ?', { query })
        if result == 0 then
            UniqueFound = true
        end
    end

	MySQL.prepare('UPDATE `users` SET `phone_number` = ? WHERE `identifier` = ?', {
		PhoneNumber,
		identifier
	})
...
ESX.GetPlayerByPhone = function(number)
    for src, val in pairs(ESX.Players) do
        if val.phone_number == number then
            return val
        end
    end
    return nil
