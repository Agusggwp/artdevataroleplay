...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('https://ttyy_phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('https://ttyy_phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(QB.Phone.Data.PlayerData.firstName+"."+QB.Phone.Data.PlayerData.lastName+"@qbcore.com");
...
$(document).on('click', '#advert-sendmessage-chat', function(e){
    ...
    if (Advert !== "") {
        ...
        if (!picture){
            $.post('https://ttyy_phone/PostAdvert', JSON.stringify({
                message: Advert,
                url: null
            }));
            ClearInputNew()
...
$(document).on('click','#new-advert-photo',function(e){
    e.preventDefault();
    $.post('https://ttyy_phone/TakePhoto',function(url){
        if(url){
...
QB.Phone.Functions.RefreshAdverts = function(Adverts) {
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $.each(Adverts, function(i, advert){
            ...
            if (advert.number === QB.Phone.Data.PlayerData.phonedata.phone_number){
...
$(document).on('click','#adv-delete',function(e){
    e.preventDefault();
    $.post('https://ttyy_phone/DeleteAdvert', function(){
        setTimeout(function(){
