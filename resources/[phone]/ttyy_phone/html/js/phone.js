...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('https://ttyy_phone/ClearRecentAlerts');
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    $.post('https://ttyy_phone/CallContact', JSON.stringify({
        ContactData: cData,
        Anonymous: QB.Phone.Data.AnonymousCall,
    }), function(status){
        if (cData.number !== QB.Phone.Data.PlayerData.phonedata.phone) {
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
...
$(document).on('click', "#phone-number-call-free-btn", function(e){
    ...
    if (InputNum != ""){
        ...
        $.post('https://ttyy_phone/CallContact', JSON.stringify({
            ...
            if (cData.number !== QB.Phone.Data.PlayerData.phonedata.phone_number) {
                if (status.IsOnline) {
                    if (status.CanCall) {
                        if (!status.InCall) {
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== QB.Phone.Data.PlayerData.phonedata.phone_number) {
        ...
        setTimeout(function(){
            ...
            $.post('https://ttyy_phone/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
...
$(document).on('click', '#phone-number-savecontact-edit', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://ttyy_phone/EditContact', JSON.stringify({
...
$(document).on('click', '#delete-contact', function(e){
    ...
    $.post('https://ttyy_phone/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#phone-number-savecontact', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('https://ttyy_phone/AddNewContact', JSON.stringify({
        ...
        if (SelectedSuggestion !== null) {
            $.post('https://ttyy_phone/RemoveSuggestion', JSON.stringify({
...
$(document).on('click', '#outgoing-cancel', function(e){
    ...
    $.post('https://ttyy_phone/CancelOutgoingCall');
});

$(document).on('click', '#incoming-deny', function(e){
    e.preventDefault();

    $.post('https://ttyy_phone/DenyIncomingCall');
});

$(document).on('click', '#ongoing-cancel', function(e){
    e.preventDefault();

    $.post('https://ttyy_phone/CancelOngoingCall');
});
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('https://ttyy_phone/AnswerCall');
    $("#incoming-answer").css({"display":"none"});
});
