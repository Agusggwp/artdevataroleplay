    ...
    if (CurrentHouseData.keyholders !== undefined && CurrentHouseData.keyholders !== null) {
        $.each(CurrentHouseData.keyholders, function(i, keyholder){
            if (keyholder !== null && keyholder !== undefined) {
                var elem;
                if (keyholder.identifier !== QB.Phone.Data.PlayerData.identifier) {
                    elem = '<div class="house-key" id="holder-'+i+'"><span class="house-key-holder">' + keyholder.firstName + ' ' + keyholder.lastName + '</span> <div class="house-key-delete"><i class="fas fa-trash"></i></div> </div>';
                } else {
                    elem = '<div class="house-key" id="holder-'+i+'"><span class="house-key-holder">(Me) ' + keyholder.firstName + ' ' + keyholder.lastName + '</span></div>';
                } 
...
$(document).on('click', '.house-key-delete', function(e){
    ...
    $.post('https://ttyy_phone/RemoveKeyholder', JSON.stringify({
        HolderData: Data,
...
$(document).on('click', '#myhouse-option-transfer-confirm', function(e){
    ...
    $.post('https://ttyy_phone/TransferCid', JSON.stringify({
        newBsn: NewBSN,
        HouseData: CurrentHouseData,
