...
$(document).on('click', '.racing-race', function(e){
    ...
    var IsRacer = IsInRace(QB.Phone.Data.PlayerData.identifier, RaceData.RaceData.Racers)
...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            ...
            var InRace = IsInRace(QB.Phone.Data.PlayerData.identifier, race.RaceData.Racers);
...
$(document).on('click', '#join-race', function(e){
    ...
    $.post('https://ttyy_phone/IsInRace', JSON.stringify({}), function(IsInRace){
        if (!IsInRace) {
            $.post('https://ttyy_phone/RaceDistanceCheck', JSON.stringify({
                RaceId: Data.RaceId,
                Joined: true,
            }), function(InDistance){
                if (InDistance) {
                    $.post('https://ttyy_phone/IsBusyCheck', JSON.stringify({
                        check: "editor"
                    }), function(IsBusy){
                        if (!IsBusy) {
                            $.post('https://ttyy_phone/JoinRace', JSON.stringify({
                                RaceData: Data,
                            }));
...
$(document).on('click', '#quit-race', function(e){
    ...
    $.post('https://ttyy_phone/LeaveRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '#start-race', function(e){
    ...
    $.post('https://ttyy_phone/StartRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('https://ttyy_phone/GetTrackData', JSON.stringify({
        RaceId: $(this).attr('id')
    }), function(TrackData){
...
$(document).on('click', '#setup-race', function(e){
    ...
    $.post('https://ttyy_phone/GetRaces', JSON.stringify({}), function(Races){
...
$(document).on('click', '#create-race', function(e){
    e.preventDefault();
    $.post('https://ttyy_phone/IsAuthorizedToCreateRaces', JSON.stringify({}), function(data){
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        ...
        $.post('https://ttyy_phone/IsAuthorizedToCreateRaces', JSON.stringify({
            TrackName: TrackName
        }), function(data){
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    $.post('https://ttyy_phone/StartTrackEditor', JSON.stringify({
                        TrackName: TrackName
                    }));
...
$(document).on('click', '#setup-race-accept', function(e){
    ...
    $.post('https://ttyy_phone/HasCreatedRace', JSON.stringify({}), function(HasCreatedRace){
        if (!HasCreatedRace) {
            $.post('https://ttyy_phone/RaceDistanceCheck', JSON.stringify({
                RaceId: track,
                Joined: false,
                ...
                if (InDistance) {
                    if (track !== undefined || track !== null) {
                        if (laps !== "") {
                            $.post('https://ttyy_phone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    $.post('https://ttyy_phone/SetupRace', JSON.stringify({
                                        RaceId: track,
                                        AmountOfLaps: laps,
                                    }))
                                    $(".racing-overview").animate({
...
$(document).on('click', '#leaderboards-race', function(e){
    ...
    $.post('https://ttyy_phone/GetRacingLeaderboards', JSON.stringify({}), function(Races){
        if (Races !== null) {
