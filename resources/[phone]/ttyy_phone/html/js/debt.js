...
function LoadDebtJob(){
    ...
    $.post('https://ttyy_phone/GetHasBills_debt', JSON.stringify({}), function(HasTable){
...
$(document).on('click', '#debt-create-bill-ended', function(e){
    ...
    if ((ID && Amount && Reason) != "" && (ID && Amount) >= 1){
        $.post('https://ttyy_phone/SendBillForPlayer_debt', JSON.stringify({
            ID: ID,
            Amount: Amount,
            Reason: Reason,
        }));
...
$(document).on('click', '#debt-create-bill-accept', function(e){
    e.preventDefault();
    var PlyMoney = QB.Phone.Data.PlayerData.accounts[0].money;
    if(PlyMoney >= ENDamount){
        $.post('https://ttyy_phone/debit_AcceptBillForPay', JSON.stringify({
            id: ENDKey,
            Amount: ENDamount,
            CSN: ENDsenderCSN,
        }));
