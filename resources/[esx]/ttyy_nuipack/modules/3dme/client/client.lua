...
AddEventHandler('esx:nui_ready', function()
    CreateFrame('3dme', 'nui://' .. GetCurrentResourceName() .. '/modules/3dme/data/html/ui.html')
    print('3DME LOAD')
end)
    ...
    if playerId ~= -1 or source == GetPlayerServerId(PlayerId()) then
        ...
        Citizen.CreateThread(function()
                ...
                if distance < 25.0 then
                    local onScreen, xxx, yyy =
                        GetHudScreenPositionFromWorldPosition(
                            sourceCoords.x + Medo.CoordsX,
                            sourceCoords.y + Medo.CoordsY,
                            sourceCoords.z + Medo.CoordsZ)
                    htmlString =
                        htmlString ..
            ...
            if isDisplaying == false then
                SendFrameMessage('3dme',{toggle = false})
            end
        end)
        Citizen.CreateThread(function()
            Citizen.Wait(Medo.Duration)
            liczba = liczba -1
            isDisplaying = false
            SendFrameMessage('3dme',{toggle = false})
        end)
