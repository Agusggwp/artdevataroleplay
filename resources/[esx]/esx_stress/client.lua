    ...
    TriggerEvent('esx_status:getStatus','stress',function(status)
      ...
      if status.val >= (self.GetSickAt * 10000) then
        ...
        if not self.IsSick then
          self.IsSick = true
          exports['ttyy_nuipack']:Alert('error', 'You have become sick from too much stress. Try some relieving activities.')
        else
          ...
          elseif self.ExtremelySick then 
            ...
            exports['ttyy_nuipack']:Alert('inform', 'You feel a little better. Keep up the calming activities.')
          end
        end
      elseif status.val <= (self.GetSickAt * 10000) then
        if self.IsSick then
          self.IsSick = false
          exports['ttyy_nuipack']:Alert('inform', 'You feel more relaxed.')
  ...
  while true do
    ...
    TriggerEvent('esx_status:getStatus','stress', function(stress)
      if stress.val >= (MFS.ExtremelySickAt * 10000) then
        ...
        SetEntityHealth(GetPlayerPed(-1), hp - 0);
